---
title: "Bias Analysis: <br> Basic Concepts and Methods"
subtitle: "EPIB 705"
author: "Sam Harper"
date: 2026-03-18
date-format: iso
...
[^rothman-p362]: See @Rothman:2008aa, p.362.
...
:::
::: {.column width="85%"}
- "Bias analysis requires educated guesses about the likely sizes of systematic errors"[^ME-p347]
...
[^ME-p347]: Rothman, Greenland, Lash, *Modern Epidemiology* 3rd ed, p,347.

## Example of typical qualitative analysis

Study[^parekh-vitd] assessing the association of vitamin D on age-related macular degeneration (AMD):

[^parekh-vitd]: @Parekh:2007aa
...
::: center
Which red line is longer?[^muller-lyer-1]
:::

[^muller-lyer-1]: https://michaelbach.de/ot/sze-muelue/
...
Intuitions are difficult to overcome.[^muller-lyer-2]

[^muller-lyer-2]: https://michaelbach.de/ot/sze-muelue/
...
- Early ideas formulated in the context of controversy surrounding the link between smoking and lung cancer[^bross-cornfield]

[^bross-cornfield]: @Bross:1954aa is credited with implementing the first quantitative bias analyis, but @Cornfield:1959aa also played an important role.
...
- Suppose we want to assess the effect of occupational exposure to resins ($X$) on lung cancer risk ($D$) using a case-control design.[^greenland-resins]

[^greenland-resins]: Longstanding example used in *Modern Epidemiology* from @Greenland:1994aa
...
d_sym <- data.frame(
  " "     = c("\\(D=1\\)", "\\(D=0\\)", "Total"),
  "X=1"   = c("\\(A_{11}\\)",       "\\(B_{11}\\)",       "\\(N_{11}\\)"),
  "X=0"   = c("\\(A_{01}\\)",       "\\(B_{01}\\)",       "\\(N_{01}\\)"),
  "Total" = c("\\(M_{11}\\)",       "\\(M_{01}\\)",       "\\(N_{+1}\\)"),
  "  "    = c("", "", ""),
  "X=1"   = c("\\(A_{1+}-A_{11}\\)", "\\(B_{1+}-B_{11}\\)", "\\(N_{1+}-N_{11}\\)"),
  "X=0"   = c("\\(A_{0+}-A_{01}\\)", "\\(B_{0+}-B_{01}\\)", "\\(N_{0+}-N_{01}\\)"),
...
tt(d_sym, escape = FALSE) |>
  group_tt(j = list("Z=1 Smokers" = 2:4, 
    "Z=0 Non-smokers" = 6:8, "Total" = 10:12)) |>
  style_tt(j = 1:12, fontsize = 0.9)
```
...
- However, $A_{11}$ and $A_{01}$ are still unknown.

- Can be estimated if we specify a plausible value for the association between smoking $(Z)$ and disease $(D)$.
...
$${\color{blue}OR_{DZ1}=\dfrac{A_{11}\left(B_{1+}-B_{11}\right)}{\left(A_{1+}-A_{11}\right)B_{11}}},\quad \text{and} \quad {\color{red}OR_{DZ0}=\dfrac{A_{01}\left(B_{0+}-B_{01}\right)}{\left(A_{0+}-A_{01}\right)B_{01}}}$$

## Estimating $Z$ among cases

Recall our stratum-specific $ORs$:

$$OR_{DZ1}=\dfrac{A_{11}\left(B_{1+}-B_{11}\right)}{\left(A_{1+}-A_{11}\right)B_{11}}\quad \text{and} \quad OR_{DZ0}=\dfrac{A_{01}\left(B_{0+}-B_{01}\right)}{\left(A_{0+}-A_{01}\right)B_{01}}$$

- If we can substitute reasonable values for these $z$-specific $OR$s, we can solve[^rothman-p350] the above equations to get the value of $A_{11}$ and $A_{01}$:

[^rothman-p350]: See @Rothman:2008aa, p.350, eq. 19-1 and 19-2
...
- Suppose external studies show a 70% prevalence of smoking among those exposed to resins and 50% among those unexposed.[^just] We can estimate $B_{11}$ and $B_{01}$ as:

[^just]: This choice needs justification and should be defended on substantive grounds. See @Lash:2014aa.
...
d_worked <- data.frame(
  " "     = c("\\(D=1\\)", "\\(D=0\\)", "Total"),
  "X=1"   = c("\\(A_{11}\\)",     "180",              "\\(N_{11}\\)"),
  "X=0"   = c("\\(A_{01}\\)",     "473",              "\\(N_{01}\\)"),
  "Total" = c("\\(M_{11}\\)",     "\\(M_{01}\\)",     "\\(N_{+1}\\)"),
  "  "    = c("", "", ""),
  "X=1"   = c("\\(45-A_{11}\\)",  "\\(257-B_{11}\\)", "\\(302-N_{11}\\)"),
  "X=0"   = c("\\(94-A_{01}\\)",  "\\(945-B_{01}\\)", "\\(1039-N_{01}\\)"),
...
tt(d_worked, escape = FALSE) |>
  group_tt(j = list("Z=1 Smokers" = 2:4, "Z=0 Non-smokers" = 6:8, "Total" = 10:12)) |>
  style_tt(i = 1, j = 2:3, bold = TRUE) |>
  style_tt(i = 2, j = 2,  color = "red")  |>
  style_tt(i = 2, j = 3,  color = "blue") |>
  style_tt(i = 2, j = 10, color = "red")  |>
  style_tt(i = 2, j = 11, color = "blue") |>
  style_tt(j = 1:12, fontsize = 0.9)
```
...
- Plugging in the estimates of $B_{11}$ and $B_{01}$ along with a plausible estimate of the confounder-disease association $(OR_{DZ1}=OR_{DZ0}=5)$, assumed[^homogeneity] to be homogeneous across strata of resin exposure, now allow us to calculate $A_{11}$ and $A_{01}$:

[^homogeneity]: Not necessary to assume homogeneity if there is evidence to the contrary.
...
- Arah et al.[^arah-2008] also give an alternative estimator using only 1) prevalence of $Z$ in each exposure stratum and 2) association between $Z$ and $D$

[^arah-2008]: See @Arah:2008aa
...
[→ See appendix for extensions to RR, RD, and general formulas](#app-extensions)

## Simple implementation via `episensr`[^episensr-conf]

[^episensr-conf]: See package by @Haine:2021aa
...
# bias parameters (ORdz, Pz1, Pz0)
  bias_parms = c(5, 0.7, 0.5))
```
:::
::: {.column width="65%"}
```{r, echo=FALSE}
...
## All the parameters matter (not an E-value)

- What if we change the $OR_{DZ}$ from 5 to 1.5? 
...
library(episensr)
# observed data
confounders(matrix(c(45, 94, 257, 945),
  dimnames = list(c("D+", "D-"),
  c("E+", "E- ")),
  nrow = 2, byrow = TRUE),
  type = "OR",
...
Epidemiology Monitor:[^epimonitor-taubes]

[^epimonitor-taubes]: Interview in @Epimonitor:1996aa of Gary Taubes, who published "Epidemiology Faces its Limits" in *Science* 1995.
...
You may recall[^buzzfeed-covid]

[^buzzfeed-covid]: https://www.buzzfeednews.com/article/stephaniemlee/coronavirus-antibody-test-santa-clara-los-angeles-stanford
...
```{r echo=FALSE}
d_class <- data.frame(
  "Measured Status" = c("Positive", "Negative"),
  "Positive"        = c("a", "c"),
  "Negative"        = c("b", "d"),
  check.names = FALSE
)
tt(d_class) |>
  group_tt(j = list("True Status of Exposure or Disease" = 2:3))
```
...
```{r echo=FALSE}
d_trueobs <- data.frame(
  " "         = c("\\(D_1\\)", "\\(D_0\\)"),
  "\\(E_1\\)" = c("\\(A\\)", "\\(C\\)"),
  "\\(E_0\\)" = c("\\(B\\)", "\\(D\\)"),
  "  "        = c("", ""),
  "\\(E_1\\)" = c("\\(a=A(Se_{D_1})+B(1-Sp_{D_1})\\)", "\\(c=C(Se_{D_0})+D(1-Sp_{D_0})\\)"),
  "\\(E_0\\)" = c("\\(b=A(1-Se_{D_1})+B(Sp_{D_1})\\)", "\\(d=C(1-Se_{D_0})+D(Sp_{D_0})\\)"),
  check.names = FALSE
)
tt(d_trueobs, escape = FALSE) |>
  group_tt(j = list("True" = 2:3, "Expected Observed Cells" = 5:6))
```
...
```{r echo=FALSE}
d_trueobs_num <- data.frame(
  " "         = c("\\(D_1\\)", "\\(D_0\\)", "Total"),
  "\\(E_1\\)" = c("200", "800",  "1000"),
  "\\(E_0\\)" = c("100", "900",  "1000"),
  "  "        = c("", "", ""),
  "\\(E_1\\)" = c("\\(200(.85)+100(.05)=175\\)", "\\(800(.85)+900(.05)=725\\)", "900"),
  "\\(E_0\\)" = c("\\(200(.15)+100(.95)=125\\)", "\\(800(.15)+900(.95)=975\\)", "1100"),
  check.names = FALSE
)
tt(d_trueobs_num, escape = FALSE) |>
  group_tt(j = list("True" = 2:3, "Expected Observed" = 5:6))
```
...
```{r echo=FALSE}
d_obscorr <- data.frame(
  " "         = c("\\(D_1\\)", "\\(D_0\\)"),
  "\\(E_1\\)" = c("\\(a\\)", "\\(c\\)"),
  "\\(E_0\\)" = c("\\(b\\)", "\\(d\\)"),
  "Total"     = c("\\(D_{1Tot}\\)", "\\(D_{0Tot}\\)"),
  "  "        = c("", ""),
  "\\(E_1\\)" = c("\\(\\dfrac{a-D_{1Tot}(1-Sp_{D_1})}{Se_{D_1}-(1-Sp_{D_1})}\\)",
                  "\\(\\dfrac{c-D_{0Tot}(1-Sp_{D_0})}{Se_{D_0}-(1-Sp_{D_0})}\\)"),
  "\\(E_0\\)" = c("\\(D_{1Tot}-A\\)", "\\(D_{0Tot}-C\\)"),
  check.names = FALSE
)
tt(d_obscorr, escape = FALSE) |>
  group_tt(j = list("Observed" = 2:4, "Corrected" = 6:7))
```
...
3. Sample randomly:
    ...
    - Need adequate samples for rare exposures or outcomes.
:::
...
tt(d_class) |>
  group_tt(j = list("True Status" = 2:3)) |>
  style_tt(j = 1:3, fontsize = 0.75)
```
:::
...
tt(d_typeA, escape = FALSE) |>
  group_tt(j = list("True" = 2:3, "Observed Cells" = 5:7))
```
...
## Simple implementation via `episensr`[^episensr-misc]

[^episensr-misc]: See package by @Haine:2021aa
...
## Simpler implementation using PPV/NPVs[^fox-ch6-ppvnpv]

[^fox-ch6-ppvnpv]: See Ch.6 in @Fox:2022aa
...
- Estimates for Se and Sp are applied to measurement of confounder rather than exposure.[^fox-ch6-conf]

[^fox-ch6-conf]: Or both. See examples in Ch.6 of @Fox:2022aa.
...
## Example: Effect of coffee consumption on bladder cancer[^slattery-coffee]

[^slattery-coffee]: @Slattery:1988aa
...
dimnames = list(
  c("Cancer+", "Cancer-"),
  c("Coffee+", "Coffee-"), 
  c("Total", "Smoking+", "Smoking-")),
dim = c(2, 2, 3)),
bias_parms = c(.85, .85, .95, .95))
```
:::
::: {.column width="65%"}
```{r, echo=FALSE}
...
:::
::: {.column width="60%"}
Example: mobile phone use and uveal melanoma[^stang-mobile]
...
[^stang-mobile]: Example from @Stang:2009aa described in @Fox:2022aa
...
tt(d_uveal) |>
  group_tt(j = list("Participants" = 2:3, "Non-participants+Q" = 5:6))
```
...
```{r echo=FALSE}
d_ltf <- data.frame(
  " "           = c("Deaths", "Persons", "Person-years", "Crude rate", "Crude RD", "Crude RR"),
  "E+"          = c("40", "104", "687",  "5.8/100py", "3.3/100py", "2.3"),
  "E\u2212"     = c("65", "286", "2560", "2.5/100py", "0",         "1.0"),
  "  "          = c("", "", "", "", "", ""),
  "E+"          = c("", "13", "", "", "", ""),
  "E\u2212"     = c("", "46", "", "", "", ""),
  check.names = FALSE
...
tt(d_ltf) |>
  group_tt(j = list("With follow-up" = 2:3, "Lost to follow-up" = 5:6))
```
...
tt(d_ltf2) |>
  ...
  style_tt(i = 2, j = 2, color = "red")  |>
  style_tt(i = 2, j = 3, color = "blue") |>
  style_tt(i = 3, j = 2, color = "red")  |>
  style_tt(i = 3, j = 3, color = "blue") |>
  style_tt(i = 3, j = 5, color = "red")  |>
  style_tt(i = 3, j = 6, color = "blue")
```
...
```{r echo=FALSE}
d_hosp <- data.frame(
  " "         = c("Deaths", "Person-years", "Crude rate", "Crude RD", "Crude RR"),
  "E+"        = c("3",    "60.8",  "4.9/100py", "0.4/100py", "1.1"),
  "E\u2212"   = c("5",    "110.2", "4.5/100py", "0",         "1.0"),
  "  "        = c("", "", "", "", ""),
  "E+"        = c("4.2",  "85.9",  "", "", ""),
  "E\u2212"   = c("18.7", "411.7", "", "", ""),
  check.names = FALSE
...
tt(d_hosp) |>
  group_tt(j = list("At 2 hospitals" = 2:3, "Estimated for those lost" = 5:6)) |>
  style_tt(i = 1, j = 5, color = "red")  |>
  style_tt(i = 1, j = 6, color = "red")  |>
  style_tt(i = 3, j = 2, color = "blue") |>
  style_tt(i = 3, j = 3, color = "blue")
```
...
tt(d_worst) |>
  group_tt(j = list("With follow-up" = 2:3, "Lost to follow-up" = 5:6)) |>
  style_tt(i = 1, j = 5, color = "blue") |>
  style_tt(i = 1, j = 6, color = "blue")
```
...
Peri-operative consultation and 30d mortality Confounder-adjusted RR=1.16 (1.07,1.26).[^wijeysundera]
:::
::: {.column width="80%"}
```{r, out.width="100%"}
#| fig-align: center
...
[^wijeysundera]: @Wijeysundera:2010aa

## Probabilistic Bias Analysis

- The major limitation of the previous methods for analyzing study bias is that they treat the bias parameters (sensitivity, specificity, confounder-disease association, prevalence of unmeasured confounders, etc.) as [known quantities]{.blue} that are perfectly measured.
...
## Basic steps for probabilistic bias analysis[^fox-prob]

[^fox-prob]: See Ch.8 in @Fox:2022aa
...
pc <- probsens_conf(matrix(c(45, 94, 257, 945),
...
# prevalence of U among exposed
  prev_exp = list("triangular", c(.6, .8, .7)),

# prevalence of U among unexposed
  prev_nexp = list("triangular", c(.4, .6, .5)),
...
```{r, echo=FALSE}
#| fig-align: center
p1    <- data.frame(pc$sim_df$p1)
p0    <- data.frame(pc$sim_df$p0)
rrcd  <- data.frame(pc$sim_df$RR_cd)
bcor  <- data.frame(pc$sim_df$tot_OR)

pba_p1 <- ggplot(p1, aes(x = pc$sim_df$p1)) + theme_classic() +
  geom_histogram(binwidth = 0.005, color = "#5e3c99", fill = "#b2abd2") +
  labs(x = "P(Confounder in E+)", y = "No. observations")

pba_p0 <- ggplot(p0, aes(x = pc$sim_df$p0)) + theme_classic() +
  geom_histogram(binwidth = 0.005, color = "#5e3c99", fill = "#b2abd2") +
  labs(x = "P(Confounder in E-)", y = "No. observations")

pba_rrcd <- ggplot(rrcd, aes(x = pc$sim_df$RR_cd)) + theme_classic() +
...
```{r, echo=FALSE}
#| fig-align: center
ggplot(bcor, aes(x = pc$sim_df$tot_OR)) + theme_classic() +
...
```{r, echo=FALSE}
oor   <- as.numeric(gsub("\\D+", ".", pc$obs_measures[2,]))
cors  <- as.numeric(gsub("\\D+", ".", pc$adj_measures[3,]))
corsr <- as.numeric(gsub("\\D+", ".", pc$adj_measures[4,]))
bcor_t <- data.frame(
  Estimate = c("OR: Crude",
               "OR: Corrected (systematic)",
               "OR: Corrected (systematic + random)"),
  rbind(oor, cors, corsr),
  row.names = NULL
)
names(bcor_t)[2:4] <- c("Median", "2.5th pctile", "97.5th pctile")
tt(bcor_t, digits = 2)
```

- Using probabilistic analysis gives a similar adjusted OR (1.38) relative to our deterministic bias analysis (1.39), but provides empirical confidence limits.
...
## Multiple bias analysis example[^johnson-multibias]

[^johnson-multibias]: @Johnson:2018aa
...
## Basic workflow for record-level analysis[^fox-ch9]

[^fox-ch9]: See @Fox:2022aa Chapter 9.
...
Perform simple QBA:[^first-obs-row]

[^first-obs-row]: Note that the values in the table are in the first observation row.
...
- If the trial returns a 1, set $c=1$, if not, then $c=0$, i.e., does not have the confounder.
...
## The core tension in record-level QBA

- Fox et al. still collapse to a **marginal 2×2 table** at the bias-correction step — the formulas for misclassification, confounding, and selection bias were derived for cell counts

- But if your final model adjusts for $W$ and $Z$, you care about the *conditional* $X$–$Y$ association, not the marginal one — **these are different quantities**

. . .

- Matters most when bias is *differential* across levels of $W$ or $Z$ (e.g. $P(U=1 \mid X)$ varies by $W$)

- **Fix:** stratify the correction *within* strata of $W$ and $Z$ — Fox et al. call this "collapsing with reweighting"

## A cleaner approach: direct record-level simulation

Skip the contingency table entirely. For unmeasured confounding, simulate $U$ directly:

1. Specify $P(U=1 \mid X)$ and $OR_{UY}$ as bias parameters
2. Compute $P(U=1 \mid X_i, Y_i)$ per record via Bayes' theorem
3. Sample $U_i \sim \text{Bernoulli}\bigl(P(U=1 \mid X_i, Y_i)\bigr)$
4. Fit $Y \sim X + W + Z + U$ — the coefficient on $X$ is now bias-adjusted

. . .

- This is just **multiple imputation of an unobserved variable** — no PPV/NPV formulas, no merging
- Extends to differential bias: if $P(U=1 \mid X)$ varies by $W$, use $W$-stratified bias parameters and compute $P(U=1 \mid X_i, Y_i, W_i)$ in step 2
- The Fox et al. contingency-table route is one way to derive the per-record posterior — not the only way[^rl]

[^rl]: See @Brendel:2023aa and @Desai:2024aa for more recent examples
...
Results of systematic review of QBAs.[^petersen-review] Graph shows original and revised estimates for:
...
[^petersen-review]: @Petersen:2021aa
...
## Unmeasured confounding for other effect measures {#app-extensions visibility="uncounted"}

[← Back to main deck](#simplifying-formula)
...
## What about the risk difference?[^cc-illustration] {visibility="uncounted"}

[^cc-illustration]: These data are derived from a case-control study, so this is just for illustration.
...
## General formulas based on counterfactual notation {visibility="uncounted"}

- What to do for more complex, regression-based models?[^vanderweele-regression]
...
[^vanderweele-regression]: Far more likely what you'll be using in practice. See @Vanderweele:2011aa. More on this later.

$$=\sum_{x}\sum_{u}\left\{ E(Y|a_{1},x,u)-E(Y|a_{0},x,u)\right\} P(u|x,a_{1})P(x|a_{1})$$

## General formulas for unmeasured confounding {visibility="uncounted"}
...
## Complications for the general formula {visibility="uncounted"}
...
## External adjustment for unmeasured confounding {visibility="uncounted"}

Assumptions:[^rosenbaum-evalue]

[^rosenbaum-evalue]: See @Rosenbaum:1983aa for more details. Similar in some ways to the 'e-value' concept from @VanderWeele:2017aa.
...
```{r echo=FALSE}
d_rr <- data.frame(
  " " = c("Adjusted estimate (0.67--0.36)",
          "\\(\\delta=P(U|a_1)-P(U|a_0)\\) all strata",
          "\\(\\gamma=E(Y|U=1)-E(Y|U=0)\\) all strata",
          "Bias\\((\\gamma\\delta)\\)",
          "Bias-corrected effect"),
  "Enough to nullify" = c("0.31 (0.17, 0.45)", "0.6",   "0.517",  "0.310", "0.0 (−0.14, 0.14)"),
  "More plausible"    = c("0.31 (0.17, 0.45)", "0.3",   "0.5217", "0.155", "0.16 (0.01, 0.30)"),
  check.names = FALSE
)
tt(d_rr, escape = FALSE) |>
  group_tt(j = list("Rosenbaum & Rubin (1983)" = 2:3))
```
