...
:::
::: {.column width="85%"}
- "Bias analysis requires educated guesses about the likely sizes of systematic errors"[^2]
...
[^2]: Rothman, Greenland, Lash, *Modern Epidemiology* 3rd ed, p,347.

## Example of typical qualitative analysis

Study[^3] assessing the association of vitamin D on age-related macular degeneration (AMD):
...
::: center
Which red line is longer?[^4]
:::

[^4]: https://michaelbach.de/ot/sze-muelue/
...
Intuitions are difficult to overcome.[^5]

[^5]: https://michaelbach.de/ot/sze-muelue/
...
- Early ideas formulated in the context of controversy surrounding the link between smoking and lung cancer[^6]

[^6]: @Bross:1954aa is credited with implementing the first quantitative bias analyis, but @Cornfield:1959aa also played an important role.
...
- Suppose we want to assess the effect of occupational exposure to resins ($X$) on lung cancer risk ($D$) using a case-control design.[^7]

[^7]: Longstanding example used in *Modern Epidemiology* from @Greenland:1994aa
...
- If we can substitute reasonable values for these $z$-specific $OR$s, we can solve[^8] the above equations to get the value of $A_{11}$ and $A_{01}$:

[^8]: See @Rothman:2008aa, p.350, eq. 19-1 and 19-2
...
- Suppose external studies show a 70% prevalence of smoking among those exposed to resins and 50% among those unexposed. We can estimate $B_{11}$ and $B_{01}$ as:
...
- Plugging in the estimates of $B_{11}$ and $B_{01}$ along with a plausible estimate of the confounder-disease association $(OR_{DZ1}=OR_{DZ0}=5)$, assumed[^9] to be homogeneous across strata of resin exposure, now allow us to calculate $A_{11}$ and $A_{01}$:

[^9]: Not necessary to assume homogeneity if there is evidence to the contrary.
...
- Arah et al.[^10] also give an alternative estimator using only 1) prevalence of $Z$ in each exposure stratum and 2) association between $Z$ and $D$
...
Epidemiology Monitor:[^14]

[^14]: Interview in @Epimonitor:1996aa of Gary Taubes, who published "Epidemiology Faces its Limits" in *Science* 1995.
...
## Simple implementation via `episensr`[^17]

[^17]: See package by @Haine:2021aa
...
## Simpler implementation using PPV/NPVs[^18]

[^18]: See Ch.6 in @Fox:2022aa
...
- Estimates for Se and Sp are applied to measurement of confounder rather than exposure.[^19]

[^19]: See examples in Ch.6 of @Fox:2022aa.
...
## Example: Effect of coffee consumption on bladder cancer[^20]
...
:::
::: {.column width="60%"}
Example: mobile phone use and uveal melanoma[^21]
...
[^21]: Example from @Stang:2009aa described in @Fox:2022aa
...
Peri-operative consultation and 30d mortality Confounder-adjusted RR=1.16 (1.07,1.26).[^22]
:::
::: {.column width="80%"}
```{r, out.width="100%"}
#| fig-align: center
...
## Multiple bias analysis example[^23]
...
## Basic workflow for record-level analysis[^24]

[^24]: See @Fox:2022aa Chapter 9.
...
Perform simple QBA:[^25]

[^25]: Note that the values in the table are in the first observation row.
...
Results of systematic review of QBAs.[^26] Graph shows original and revised estimates for:
...
## What about the risk difference?[^11] {visibility="uncounted"}

[^11]: These data are derived from a case-control study, so this is just for illustration.
...
- What to do for more complex, regression-based models?[^12]
...
[^12]: Far more likely what you'll be using in practice. See @Vanderweele:2011aa. More on this later.
...
[^13]: See @Rosenbaum:1983aa for more details. Similar in some ways to the 'e-value' concept from @VanderWeele:2017aa.
