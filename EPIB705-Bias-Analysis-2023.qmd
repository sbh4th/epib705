...
\foreach \from/\to in {e/y, e/em, c/y, c/e, e/s, u/e, u/y, y/ym}
   \draw [->] (\from) -- (\to);
 \draw [->] (y) to [bend right=35] (s);
\end{tikzpicture}
```
...
- "Bias analysis requires educated guesses about the likely sizes of systematic errors"^[Rothman, Greenland, Lash, *Modern Epidemiology* 3rd ed, p,347.] 
...
- Goals:
  ...
  - We want to provide an estimate of the association corrected for the
unmeasured confounder ($Z$).
  
  - Answers the question of what the association would have been **had we controlled for** $Z$.

## Unmeasured Confounding
- Suppose we want to assess the effect of occupational exposure to resins ($X$) on lung cancer risk ($D$) using a case-control design.^[Longstanding example used in *Modern Epidemiology* from @Greenland:1994aa] 
...
- Early ideas formulated in the context of controversy surrounding the link between smoking and lung cancer^[@Bross:1954aa is credited with implementing the first quantitative bias analyis, but @Cornfield:1959aa also played an important role.]
...
\cline{2-4} \cline{3-4} \cline{4-4} \cline{6-8} \cline{7-8} \cline{8-8} \cline{10-12} \cline{11-12} \cline{12-12} 
 & {\small{}X=1} & {\small{}X=0} & {\small{}Total} &  & {\small{}X=1} & {\small{}X=0} & {\small{}Total} &  & {\small{}X=1} & {\small{}X=0} & {\small{}Total}\tabularnewline
\hline 
{\small{}D=1} & \textbf{\small{}$\mathbf{{\scriptstyle A_{11}}}$} & \textbf{\small{}$\mathbf{{\scriptstyle A_{01}}}$}  & {\small{}${\scriptstyle M_{11}}$} &  & {\small{}${\scriptstyle A_{1+}-A_{11}}$} & {\small{}${\scriptstyle A_{0+}-A_{01}}$} & {\small{}${\scriptstyle M_{A+}-M_{11}}$} &  & {\small{}${\scriptstyle A_{1+}}$} & {\small{}${\scriptstyle A_{0+}}$} & {\small{}${\scriptstyle M_{A+}}$}\tabularnewline
{\small{}D=0} & \textbf{\small{}$\mathbf{{\scriptstyle B_{11}}}$} & \textbf{\small{}$\mathbf{{\scriptstyle B_{01}}}$} & {\small{}${\scriptstyle M_{01}}$} &  & {\small{}${\scriptstyle B_{1+}-B_{11}}$} & {\small{}${\scriptstyle B_{0+}-B_{01}}$} & {\small{}${\scriptstyle M_{B+}-M_{01}}$} &  & {\small{}${\scriptstyle B_{1+}}$} & {\small{}${\scriptstyle B_{0+}}$} & {\small{}${\scriptstyle M_{B+}}$}\tabularnewline
\hline 
{\small{}Total} & {\small{}${\scriptstyle N_{11}}$} & {\small{}${\scriptstyle N_{01}}$} & {\small{}${\scriptstyle N_{+1}}$} &  & {\small{}${\scriptstyle N_{1+}-N_{11}}$} & {\small{}${\scriptstyle N_{0+}-N_{01}}$} & {\small{}${\scriptstyle N_{++}-N_{+1}}$} &  & {\small{}${\scriptstyle N_{1+}}$} & {\small{}${\scriptstyle N_{0+}}$} & {\small{}${\scriptstyle N_{++}}$}\tabularnewline
& & & & & & & & & & & \tabularnewline 
\end{tabular}
...
-   With plausible estimates of $P_{Z}$, perhaps from some externally
    validated source, we can use this to estimate the
    number of exposed controls, since $B_{11}=P_{Z1}B_{1+}$ and
    $B_{01}=P_{Z0}B_{0+}$.
...
- Based on the table above, the confounder-disease OR within strata of
    $X$ $\left(OR_{DZX}\right)$ can be calculated as:
...
## Estimating exposed and unexposed cases
...
-   If we can substitute reasonable values for these $z$-specific $OR$s,
    we can solve the above equations, we can solve these equations to
    get the value of $A_{11}$and $A_{01}$:^[See @Rothman:2008aa, p.350, eq. 19-1 and 19-2]
...
-   Plugging in the estimates of $B_{11}$ and $B_{01}$ along with a
    plausible estimate of the confounder-disease association
    $(OR_{DZ1}=OR_{DZ0}=5)$, assumed^[Not necessary to assume homogeneity if there is evidence to the contrary.] to be homogeneous across strata of
    resin exposure, now allow us to calculate $A_{11}$ and $A_{01}$:
...
-   Arah et al.^[See @Arah:2008aa] also give an alternative estimator using only 1) prevalence of $Z$ in each exposure stratum and 2) association between $Z$ and $D$

$Bias(OR)=\dfrac{OR_{DX+}}{OR_{DXZ(E)}}=\dfrac{OR_{DZ0}P_{11}+1-P_{11}}{OR_{DZ0}P_{10}+1-P_{10}}=\dfrac{5(.7)+1-.7}{5(.5)+1-.5}=1.27$

where $P_{11}=P(Z=1|X=1)$, $P_{10}=P(Z=1|X=0)$, and $OR_{DZ0}$ is the
Z-D association in the unexposed.
...
-   [Choose]{style="color: red"} $RD$ for $Z\rightarrow D$ among the unexposed as
    $RD_{DZ0}=0.10$.
...
$Bias=RD_{DX+}-RD_{DXZ(E)}=RD_{DZ0}\left(P_{Z1}-P_{Z0}\right)=0.10\left(0.7-0.5\right)=0.02$

## General formulas based on counterfactual notation

- What to do for more complex, regression-based models?^[Far more likely what you'll be using in practice. See @Vanderweele:2011aa.]

-   VanderWeele and Arah (2011) provided some general formulas.

-   The average causal effect among those exposed, which **would** be
    adjusted for both measured $X$s **and** unmeasured $U$ is:
...
-   The bias is (still) a function of the prevalence of the unknown
    confounder and its effect on the outcome (reference level=$u'$):
...
-   The $U-Y$ relationship among the unexposed:
...
-   These formulas require a lot of knowledge.
    ...
    -   Association between $U$ and $Y$ [at each level of measured
        confounders]{style="color: red"} $X$.

    -   Association between $U$ and $A$ [at each level of measured
        confounders]{style="color: red"} $X$.
...
-   Even for 2 age groups and gender, for example, you would need to
    specify the $U-Y$ and $U-A$ relations for:
...
Simplifying assumptions could be useful. Assuming no heterogeneity
across $X$, we could posit:
...
Assumptions:^[See @Rosenbaum:1983aa for more details.]

-   Differences in the distribution of $U$ by exposure similar across
    all strata of measured $X$s.
...
-   The major limitation of the previous methods for analyzing study
    bias is that they treat the bias parameters (sensitivity,
    specificity, confounder-disease association, prevalence of
    unmeasured confounders, etc.) as known quantities that are perfectly
    measured.

-   Thus, the analyses above are referred to as deterministic, and they
    only account for systematic error (i.e., they do not account for
    measurement error in the estimates of the prevalence of the
    unmeasured confounder or the confounder-disease association.

-   An alternative would be to assign a probability distribution to bias
    parameters.
...
- Using probabilistic analysis gives a similar adjusted OR () relative to our deterministic bias analysis (), but provides empirical confidence limits. 

- Introduction of additional random error via the simulation analysis widens the confidence limits further.

## Record level implementation

- Benefit of adjustment for record-level covariates

## Steps for record-level implementatio
...
-   Extension of simple bias analysis in which we assign bias
    parameters, either deterministically or probabilistically, but now
    the we examine the impact of more than one bias at a time.

-   Difficult to ascertain quantitatively how multiple biases may work
    together.

-   Order matters, and corrections should be made in the reverse of the
    order in which they occurred as the data were generated.
